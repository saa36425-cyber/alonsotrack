...
app.put('/api/maintenances/:id', authAdmin, (req, res) => {
  const idx = db.maintenances.findIndex(m => m.id === req.params.id);
  if (idx === -1) return res.status(404).json({ error: 'No encontrado' });
  db.maintenances[idx] = { ...db.maintenances[idx], ...req.body };
  if (req.body.km) {
    const v = db.vehicles.find(v => v.id === db.maintenances[idx].vehicle_id);
    if (v) v.current_km = req.body.km;
  }
  saveDB(db);
  res.json({ ok: true });
});
